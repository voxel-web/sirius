...
describe SiriusApi::PeopleAuthorizer do
  ...
  describe '#authorize_request!' do
    ...
    context 'for READ_PERSONAL scope' do
      it 'allows request to personal endpoint' do
        expect { authorizer.authorize_request!(:get, '/people/:username', {username: 'ALICE'}) }.not_to raise_error
      end

      it 'denies request to endpoint for other user' do
        expect { authorizer.authorize_request!(:get, '/people/:username', {username: 'BOB'}) }.to raise_error(SiriusApi::Errors::Authorization)
      end

      it 'denies request to the index endpoint' do
        expect { authorizer.authorize_request!(:get, '/people') }.to raise_error(SiriusApi::Errors::Authorization)
      end
    end
    ...
    context 'for READ_ALL scope' do
      ...
      it 'allows request to individual endpoints' do
        expect { authorizer.authorize_request!(:get, '/people/:username', {username: 'BOB'}) }.not_to raise_error
      end
